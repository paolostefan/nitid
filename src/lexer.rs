...
///
/// # Error handling
/// All errors are returned as a [`Diagnostic`] with source location.
use crate::ast::Span;
...
impl TokenKind {
    pub fn c_str(&self) -> Cow<'static, str> {
        match self {
            ...
            Self::Opaque => Cow::Borrowed("opaque"),
...
impl Lexer {
                    ...
                    Some('u') => {
                        ...
                        let c = char::from_u32(code).ok_or_else(|| {
                            Diagnostic::new(
                                Phase::Lex,
                                Span::new(&self.file, start_line, start_col),
                                format!("Invalid Unicode escape \\u{:04X}", code),
                            )
                        })?;
                        s.push(c);
                    }
                    Some('U') => {
                        ...
                        let c = char::from_u32(code).ok_or_else(|| {
                            Diagnostic::new(
                                Phase::Lex,
                                Span::new(&self.file, start_line, start_col),
                                format!("Invalid Unicode escape \\U{:08X}", code),
                            )
                        })?;
                        s.push(c);
                    }
    ...
    fn read_char(&mut self) -> Result<Token, Diagnostic> {
        let start_line = self.line;
        let start_col = self.col;
                ...
                Some('u') => {
                    ...
                    if code > 0xFF {
                        return Err(Diagnostic::new(
                            Phase::Lex,
                            Span::new(&self.file, start_line, start_col),
                            format!("Unicode escape \\u{:04X} out of range for char", code),
                        ));
                    }
                ...
                Some('U') => {
                    ...
                    if code > 0xFF {
                        return Err(Diagnostic::new(
                            Phase::Lex,
                            Span::new(&self.file, start_line, start_col),
                            format!("Unicode escape \\U{:08X} out of range for char", code),
                        ));
                    }
                    char::from_u32(code).ok_or_else(|| {
                        Diagnostic::new(
    ...
    fn read_hex_digits(&mut self, n: usize) -> Result<u32, Diagnostic> {
        ...
        for _ in 0..n {
            match self.advance() {
                ...
                Some(c) => {
                    return Err(Diagnostic::new(
                        Phase::Lex,
                        Span::new(&self.file, start.0, start.1),
                        format!("Expected {} hex digits in Unicode escape, got '{}'", n, c),
                    ));
                }
                None => {
                    return Err(Diagnostic::new(
                        Phase::Lex,
                        Span::new(&self.file, start.0, start.1),
                        format!("Unterminated Unicode escape, expected {} hex digits", n),
                    ));
                }
            }
        }
    ...
    fn read_comment_block(&mut self) -> Result<(), Diagnostic> {
