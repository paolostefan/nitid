...
impl Scope {
    ...
    fn declare(&mut self, name: &str, typ: Type, span: &Span) -> Result<(), Diagnostic> {
...
impl Sema {
    ...
    fn analyze_decls(&mut self, decls: &mut [Decl]) -> Result<(), Diagnostic> {
            ...
            match decl {
                Decl::StructDecl(s) => {
                    ...
                    if self.struct_defs.contains_key(&name) {
                        return Err(Diagnostic::new(
                            Phase::Sema,
                            Span::new(&s.span.file, s.span.line, s.span.col),
                ...
                Decl::EnumDecl(e) => {
                    ...
                    if self.enum_defs.contains_key(&name) {
                        return Err(Diagnostic::new(
                            Phase::Sema,
                            Span::new(&e.span.file, e.span.line, e.span.col),
                    ...
                    for v in &e.variants {
                        if !seen.insert(v.name.clone()) {
                            return Err(Diagnostic::new(
                                Phase::Sema,
                                Span::new(&v.span.file, v.span.line, v.span.col),
                        ...
                        next_val = val.checked_add(1).ok_or_else(|| {
                            Diagnostic::new(
                                Phase::Sema,
                                Span::new(&v.span.file, v.span.line, v.span.col),
                                format!("Enum '{}' variant values overflow", name),
                            )
                        })?;
                    }
                ...
                Decl::ImplBlock(imp) => {
                    ...
                    if !self.struct_defs.contains_key(struct_name) {
                        return Err(Diagnostic::new(
                            ...
                            format!("Cannot define methods for unknown struct '{}'", struct_name),
                        ));
                    }
                ...
                match op {
                    UnOp::Deref => match expr_type {
                        ...
                        _ => Err(Diagnostic::new(
                            Phase::Sema,
                            span.clone(),
                            format!("deref operation on non-ptr ({})", expr_type),
                        )),
                    },
                        ...
                        if !ok {
                            ...
                            return Err(Diagnostic::new(
                                ...
                                format!(
                                    ...
                                    field_name, decl_field.typ, init_type
                                ),
                            ));
                        }
                    }
    ...
    fn pop_uninit_scope(&mut self) -> Result<(), Diagnostic> {
    ...
    for name in &v.names {
        let typ = if let Some(ref t) = v.typ {
            ...
            return Err(Diagnostic::new(
                Phase::Sema,
                Span::new(&v.span.file, v.span.line, v.span.col),
                format!("Variable '{}' has no type and no initializer", name),
            ));
        };
                ...
                if is_numeric_type(init_r) && is_numeric_type(typ_r) {
                    ...
                    if !literal_fits_target(init, &typ) {
                        return Err(Diagnostic::new(
                            Phase::Sema,
                            Span::new(&v.span.file, v.span.line, v.span.col),
                            format!("Value does not fit in '{}' of type {:?}", name, typ),
                        ));
                    }
...
fn eval_enum_value(expr: &Expr, span: &Span) -> Result<i128, Diagnostic> {
    match expr {
