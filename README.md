...
The Makefile orchestrates `cargo doc --no-deps` and `mdbook build docs/`, then merges the Rust API reference under `docs/book/api/`.
...
When the code in a sourcefile is dangling, that is, not wrapped in `fn functionName() {...}`,
the transpiler automatically creates a `main` function and assigns all the dangling code to it.

For safety, a return value of zero is added: this avoids unwanted scenarios, e.g.
`main()` returns an undefined value like `-1234` and a script checks if the executable
errored by testing its retval against 0.
...
|--------------------------|-----------|------------------------------------------------------------|
| Lexer & Parser           | ✅ Done    | Recursive-descent parser, precedence climbing              |
| AST definitions          | ✅ Done    | Full expression/statement AST                              |
| Semantic analysis        | ✅ Partial | Variable scoping, return-count checks, basic type checking |
| C codegen                | ✅ Done    | Full C output with CMake support                           |
| Multiple return values   | ✅ Done    | Desugared to output pointer params in C                    |
| `println` / `printf`     | ✅ Done    | Mapped to C `printf`                                       |
| String runtime           | ✅ Partial | `nitid_string` exists but is minimal                        |
| `if` / `while`           | ✅ Done    | Full support                                               |
| Comments (`//`, `/* */`) | ✅ Done    |                                                            |
| Hex literals             | ✅ Done    |                                                            |
| Package declarations     | ✅ Done    | Parsed, used for CMake project name                        |
| `import` syntax          | ✅ Done    | Parsed but **not resolved** — no module loader             |
...
| **`for` loops**                                             | ❌ Missing  | `for` is a lexer keyword but the parser does not handle it.                                                                                              |
| **Arrays / slices**                                         | ❌ Missing  | No array or slice types in the language. Only scalar and string types exist.                                                                             |
| **Structs / enums**                                         | ❌ Missing  | No user-defined composite types.                                                                                                                         |
