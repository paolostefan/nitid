...
fn v0_1_1_basic_import_resolution() {
    ...
    let (program, _c_files, _cmake) =
        common::compile_package_main(&dir).expect("v0.1.1-basic-import should compile");
...
fn v0_1_1_import_finds_package_dir() {
    let dir = common::package_path("v0.1.1-basic-import");
    let c_out = common::get_package_c_output(&dir).expect("should produce C output");
...
fn v0_1_1_import_from_existing_sample() {
    ...
    let content = std::fs::read_to_string(&main).expect("samples/package/main.nt should exist");
...
fn v0_1_2_qualified_function_call() {
    ...
    let c_out = common::get_package_c_output(&dir).expect("v0.1.2-qualified-access should compile");
...
fn v0_1_2_qualified_struct_in_c() {
    let dir = common::package_path("v0.1.2-qualified-access");
    let c_out = common::get_package_c_output(&dir).expect("should compile");
...
fn v0_1_2_multiple_functions_from_package() {
    let dir = common::package_path("v0.1.2-qualified-access");
    let (program, _c_files, _cmake) = common::compile_package_main(&dir).expect("should compile");
...
fn v0_1_3_alias_basic() {
    ...
    let (program, _c_files, _cmake) =
        common::compile_package_main(&dir).expect("v0.1.3-import-alias should compile");
...
fn v0_1_4_transitive_import() {
    ...
    let (program, _c_files, _cmake) =
        common::compile_package_main(&dir).expect("v0.1.4-transitive should compile");
...
fn v0_1_4_transitive_c_output_has_both_packages() {
    let dir = common::package_path("v0.1.4-transitive");
    let (_, c_files, _cmake) = common::compile_package_main(&dir).expect("should compile");
...
fn v0_1_4_dependency_graph_dfs() {
    ...
    let dir = common::package_path("v0.1.4-transitive");
    let c_out = common::get_package_c_output(&dir).expect("transitive chain should compile");
...
";
    ...
    assert!(
        err.message.contains("could not find package"),
...
fn v0_1_5_package_name_mismatch_error() {
    ...
    assert!(
        err.message.contains("does not match expected package"),
...
fn v0_1_6_no_c_name_collision() {
    ...
    let content = std::fs::read_to_string(&main).expect("samples/package/main.nt should exist");
    let (_, c_files, _) =
        nitid::compile(&main, &content, "").expect("existing package sample should compile");
